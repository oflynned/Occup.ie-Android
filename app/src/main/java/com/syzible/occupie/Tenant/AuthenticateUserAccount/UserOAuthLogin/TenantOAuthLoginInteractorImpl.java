...
public class TenantOAuthLoginInteractorImpl implements TenantOAuthLoginInteractor {
    private TenantOAuthLoginView view;

    TenantOAuthLoginInteractorImpl(TenantOAuthLoginView view) {
        this.view = view;
    }

    private TenantOAuthLoginView getView() {
        if (view == null)
    ...
    public void cacheOAuthIdentity(Context context, String provider, String oauthId, String accessToken, String forename, String surname) {
        OAuthUtils.saveId(oauthId, Target.user, context);
        OAuthUtils.saveToken(accessToken, Target.user, context);
        OAuthUtils.saveProvider(provider, Target.user, context);
    ...
    public void requestAccount(Context context, JSONObject payload) {
        RestClient.get(context, Endpoints.CHECK_USER_EXISTS, new BaseJsonHttpResponseHandler<JSONObject>() {
            ...
            public void onSuccess(int statusCode, Header[] headers, String rawJsonResponse, JSONObject response) {
                try {
                    LocalPrefs.setStringPref(context, LocalPrefs.Pref.user_id, response.getString("_id"));
